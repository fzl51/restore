...
tempPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "temp")
...
输入照片，去背景照片1
'''
examples = [[os.path.dirname(__file__) + './images/1.png'],
            [os.path.dirname(__file__) + './images/2.png'],
            [os.path.dirname(__file__) + './images/3.png'],
            [os.path.dirname(__file__) + './images/4.png'],
            [os.path.dirname(__file__) + './images/5.png']]
...
with gr.Blocks(title=title, css=css_style) as demo:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            img_input = gr.components.Image(label="图片", type="numpy")
            w_option = gr.Number(label="修复程度", value=70)
            quality_option = gr.Number(label="质量", value=100)
            base64_option = gr.components.Radio(label="base64", choices=[yes, no], value=yes)

            btn = gr.Button("图像修复")
        with gr.Column(scale=3):
            # img_output = gr.components.Image(label="图片", height=600)
            # img_output = gr.text.Textbox(label="", lines=20)

            img_output = gr.components.Textbox(label="图片数据", type="text", lines=20)
    inputs = [img_input, w_option, quality_option, base64_option]
